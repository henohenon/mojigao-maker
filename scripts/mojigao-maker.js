...
const koteitrans={
  "顔のみ-顔": {size:[300,300],pos:[0,0]},
  "顔と装飾-顔": {size:[240,240],pos:[30,30]},
  "顔と装飾-装飾": {size:[300,300],pos:[0,0]}
};
var persons = {
...
const sosyoku_URLs=[
{name:"麦わら帽子",URL:"https://raw.githubusercontent.com/henoheTK/mojigao-maker/master/images/sosyoku/%E9%BA%A6%E3%82%8F%E3%82%89%E5%B8%BD%E5%AD%90.png"}
]
const karada_URLs={
...
JikkouButton.onclick = () => {
  ...
  let img_pos=images_pos= [koteitrans["顔のみ-顔"].pos].concat(mojipos[kaotype]);
  let img_size=images_pos= [koteitrans["顔のみ-顔"].size].concat(mojisize[kaotype]);
  
  loadimages(images,img_kurinuki,img_pos,img_size);
  if(sosyokuNum!==0){
    sleep(1,function () {
      ClearCanvas();
      console.log(sosyoku_URLs[sosyoku_URLs-1]);
      images=[URLtoImage(mojigaoURL),URLtoImage(sosyoku_URLs[sosyokuNum-1].URL)];
      img_kurinuki=[[300,300],[400,400]];
      img_pos=[koteitrans["顔と装飾-顔"].pos,koteitrans["顔と装飾-装飾"].pos];
      img_size=[koteitrans["顔と装飾-顔"].size,koteitrans["顔と装飾-装飾"].size];
      loadimages(images,img_kurinuki,img_pos,img_size);
    });
  }
...
function DrowResults(images,img_kurinuki,img_pos,img_size) {// 各画像を順番に描画関数
  ...
  console.log("なんでや",img_kurinuki,img_pos,img_size);
}
